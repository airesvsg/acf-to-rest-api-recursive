...
if (!class_exists('ACF_To_REST_API_Recursive')) {
    class ACF_To_REST_API_Recursive
    {
        ...
        public static $postRecursionDepth = 2;
        ...
        public static function get_fields_recursive($item, $depth)
        {
            if (is_object($item)) {
                ...
                if ($fields && static::$recursedPosts[$item->ID] < static::$postRecursionDepth) {
                    $item->acf = $fields;
                    array_walk_recursive($item->acf, $depth, array(__CLASS__, 'get_fields_recursive'));
                }
            }
        }
    }
